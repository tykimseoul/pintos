...
#ifndef VM
#define allocate_frame(a, b, c) palloc_get_page(b)
#define free_frame(a) palloc_free_page(a)
#endif
...
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage, uint32_t read_bytes, uint32_t zero_bytes, bool writable) {
    ...
    while (read_bytes > 0 || zero_bytes > 0) {
        ...
        uint8_t *kpage = allocate_frame(upage, PAL_USER, false);
        if (kpage == NULL)
            return false;
...
static bool setup_stack(void **esp) {
    ...
    kpage = allocate_frame(PHYS_BASE - PGSIZE, PAL_USER | PAL_ZERO, false);
    if (kpage != NULL) {
